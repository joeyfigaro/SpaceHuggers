...
import { resetGame, players } from "./appLevel";
import { Prop } from "./appObjects";
import { Player, Enemy } from "./appCharacters";

const clampCamera = !debug;
// const lowGraphicsSettings = (glOverlay = !window["chrome"]); // only chromium uses high settings
const lowGraphisSettings = false;
...
engineInit(
  () => {
    resetGame();
    setCameraScale(startCameraScale);
  },
  ...
  () => {
    ...
    p > 0 &&
      mainContext.fillText(
        "(originally) A JS13K Game by Frank Force",
        mainCanvas.width / 2,
        210,
      );
    ...
    p > 0 &&
      mainContext.fillText(
        "improved drastically by Shovel & Sandbox",
        mainCanvas.width / 2,
        210,
      );
